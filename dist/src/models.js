...
const INTELLIGENCE_BUCKETS = new Set([
    "high",
    "medium",
    "low",
]);
...
const MODEL_KB_PROVIDERS = new Set(MODEL_CATALOG_PROVIDER_IDS);
 ...
 */
export const MODEL_COMPARATORS = {
    runway: {
...
export function createModelsResponse(quota, options = {}) {
    const catalog = options.catalog ?? MODEL_CATALOG;
    validateModelCatalog(catalog);
    const providers = new Map(quota.providers.map((provider) => [provider.provider, provider]));
    ...
    const unmatchedWindowIds = quota.providers.flatMap((provider) => unmatchedModelWindowIds(provider, catalog.entries));
    if (!options.sort) {
        return {
    ...
    const sorted = [...models].sort((left, right) => comparator.compare(left, right) || compareModelIdentity(left, right));
    return {
        generatedAt: quota.generatedAt,
        schemaVersion: 1,
        catalog: catalogSummary(catalog),
 ...
 */
export function compareModelsByRunway(left, right) {
    ...
    if (leftRank !== 0)
        return 0;
    return ((right.effective?.runway?.usableRunwaySeconds ?? 0) -
...
export function validateModelCatalog(catalog) {
    ...
    if (!/^\d{4}-\d{2}-\d{2}$/.test(catalog.version) ||
    ...
    for (const entry of catalog.entries) {
        if (!MODEL_KB_PROVIDERS.has(entry.provider)) {
            throw new Error(`model catalog provider is unsupported: ${entry.provider}`);
        }
        ...
        if (!INTELLIGENCE_BUCKETS.has(entry.intelligence)) {
            throw new Error(`model catalog intelligence is invalid: ${entry.intelligence}`);
        }
        const key = `${entry.provider}/${entry.id}`;
        if (seen.has(key))
            throw new Error(`duplicate model catalog entry: ${key}`);
        seen.add(key);
...
function modelRecord(entry, provider) {
...
function availabilityFor(entry, provider) {
    const availability = provider.quotaSemantics?.effectiveAvailability ?? [];
    for (const windowId of entry.windowIds ?? []) {
        const found = availability.find((candidate) => candidate.scope === normalizedModelScope(windowId));
        if (found)
            return found;
    }
    return availability.find((candidate) => candidate.scope === "all_models" || candidate.scope === "all_products");
}
function unmatchedModelWindowIds(provider, entries) {
    const knownScopes = new Set(entries
...
function normalizedModelScope(windowId) {
    return windowId.replace(/_\d+$/, "").replace(/:(?:5h|7d|window:[^:]+)$/, "");
}
function stateSummary(provider) {
...
function compareModelIdentity(left, right) {
    return (left.provider.localeCompare(right.provider) ||
        left.id.localeCompare(right.id));
}
function runwayRank(model) {
