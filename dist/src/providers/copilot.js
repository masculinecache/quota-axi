...
import { clampPercent, nowIso, percentRemaining, retryAfterToIso, } from "../lib/time.js";
...
export function normalizeCopilotUser(raw) {
    const data = objectValue(raw);
    if (!data)
        return undefined;
    const windows = normalizeQuotaSnapshots(objectValue(data.quota_snapshots), data.quota_reset_date_utc);
    const plan = stringValue(data.copilot_plan) ??
        stringValue(data.access_type_sku) ??
        stringValue(data.sku);
    const accountId = stringValue(data.login);
    if (windows.length === 0 && !plan && !accountId)
        return undefined;
    return {
        plan,
...
async function fetchCopilotUser(credentials) {
    ...
    try {
        ...
        if (!quota)
            throw new Error("GitHub Copilot quota unavailable");
        return quota;
    }
    finally {
        clearTimeout(timer);
...
function normalizeQuotaSnapshots(snapshots, resetFallback) {
    if (!snapshots)
        return [];
    const windows = [];
    for (const [id, value] of Object.entries(snapshots)) {
        ...
        windows.push({
            ...
            resetsAt: parseEpochSecondsOrMillis(item.quota_reset_at) ??
                parseEpochSecondsOrMillis(resetFallback),
        });
    }
    return windows;
}
function readCredentialState(authFile = copilotAppsFile()) {
...
function extractCredentialState(raw, path) {
    ...
    const selected = candidates.find(({ host }) => host && matchesUserEndpoint(host)) ??
...
function credentialHost(key, item) {
    return (normalizeHost(stringValue(item?.host)) ??
...
function matchesUserEndpoint(host) {
    return (host === USER_HOST ||
        (USER_HOST === "api.github.com" && host === "github.com"));
}
function copilotAppsFile() {
    if (process.env.GITHUB_COPILOT_APPS_JSON)
        return process.env.GITHUB_COPILOT_APPS_JSON;
    if (process.platform === "win32") {
        return join(process.env.LOCALAPPDATA || join(homedir(), "AppData", "Local"), "github-copilot", "apps.json");
    }
    return join(process.env.XDG_CONFIG_HOME || join(homedir(), ".config"), "github-copilot", "apps.json");
}
...
function rateLimitSignal(response) {
    ...
    if (remaining === "0") {
        return {
            limited: true,
            retryAfter: parseEpochSecondsOrMillis(response.headers.get("x-ratelimit-reset")),
        };
    }
    return { limited: false };
}
function parseEpochSecondsOrMillis(value) {
    ...
    if (Number.isFinite(parsed))
        return parseEpochSecondsOrMillis(parsed);
