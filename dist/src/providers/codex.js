...
export function normalizeCodexUsage(raw) {
    ...
    const windows = deduplicateWindowIds([
        ...windowPairFromContainer(rateLimit, "five_hour", "session", "session", "weekly", "week", "weekly"),
        ...windowPairFromContainer(objectValue(data.code_review_rate_limit), "code_review_five_hour", "code review session", "session", "code_review_weekly", "code review week", "weekly"),
...
function resolveRateLimitContainer(data) {
    return (objectValue(data.rate_limit) ??
        objectValue(data.rateLimits) ??
        objectValue(data.rate_limits) ??
        data);
}
function windowPairFromContainer(container, primaryId, primaryLabel, primaryKind, secondaryId, secondaryLabel, secondaryKind) {
    if (!container)
        return [];
    const identities = {
        ...
        unfamiliar(windowSeconds) {
            ...
            const prefix = primaryId === "five_hour" ? "window" : "code_review_window";
            return {
                id: `${prefix}:${duration}`,
    ...
    return [
        normalizeWindow(container.primary_window ?? container.primary, identities.session, identities),
        normalizeWindow(container.secondary_window ?? container.secondary, identities.weekly, identities),
...
function collectNamedRateLimitWindows(data) {
    const windows = [];
    ...
    for (const entry of additional) {
        ...
        if (!item)
            continue;
        const id = stringValue(item.metered_feature) ?? stringValue(item.limit_name);
        const label = stringValue(item.limit_name) ?? id;
        const container = objectValue(item.rate_limit);
        if (!id || !label || !container)
            continue;
...
function namedLimitWindows(id, label, container) {
    const identities = {
        session: {
            id: `model:${id}:5h`,
            label: `${label} session`,
...
function deduplicateWindowIds(windows) {
...
export function mergeAccountAndLimits(account, limits) {
...
function readCredentialState(authFile = codexAuthFile()) {
    return extractCredentialState(readJsonFileResult(authFile), authFile);
}
function extractCredentialState(raw, path) {
...
async function probeCodexCli() {
    ...
    const child = spawn(binary.path, ["-s", "read-only", "-a", "untrusted", "app-server"], {
...
function codexBinaryErrorMessage(binary) {
...
function normalizeWindow(raw, fallbackIdentity, identities) {
    ...
    if (used === undefined)
        return undefined;
    const windowSeconds = numberValue(data.limit_window_seconds) ??
        (numberValue(data.windowDurationMins) === undefined
            ? undefined
            : numberValue(data.windowDurationMins) * 60);
    const resetFromSeconds = numberValue(data.reset_after_seconds) === undefined
        ? undefined
    ...
    return withRemaining({
        ...
        resetsAt: parseEpochOrIso(data.reset_at) ??
            parseEpochOrIso(data.resetsAt) ??
            resetFromSeconds,
        windowSeconds,
    });
}
function windowIdentity(windowSeconds, fallbackIdentity, identities) {
...
function isExpiredJwtPayload(payload) {
