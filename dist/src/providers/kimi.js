...
import { createKimiCodeCliCredentialSource, KIMI_CODE_CLI_CREDENTIAL_SOURCE, } from "./kimi-code-cli-credential.js";
import { createPiKimiCredentialBroker, } from "./pi-kimi-credential.js";
...
export function createKimiAdapter(overrides = {}) {
    const dependencies = {
    ...
    return {
        ...
        fetchQuota(_options) {
            ...
            const acquisition = acquireKimiQuota(dependencies).finally(() => {
                if (inFlight === acquisition)
                    inFlight = undefined;
            });
            inFlight = acquisition;
            return acquisition;
        },
        async inspectAuth(_options) {
            let piInspection;
            try {
            ...
            const piError = piInspection === "unsupported"
            ...
            const cliError = cliInspection === "error"
                ? "credential_resolution_failed"
...
async function acquireKimiQuota(dependencies) {
    const controller = new AbortController();
    const deadline = setTimeout(() => controller.abort(), dependencies.deadlineMs);
    let attempts = [];
    try {
        const piResolution = await resolveCredential(dependencies.broker, controller.signal);
        let credential;
        ...
        if (piResolution.status === "available") {
            ...
            const cliResolution = await resolveCliCredential(dependencies.cliCredentialSource, controller.signal);
            if (cliResolution.status !== "available") {
                ...
                return failureReport(cliResolution.status === "missing" ? piFailure : cliFailure, attempts, dependencies);
            }
            credential = cliResolution.accessToken;
            credentialSource = KIMI_CODE_CLI_CREDENTIAL_SOURCE;
        }
        const payload = await requestKimiQuota(credential, controller.signal, dependencies.fetch, dependencies.now);
        const normalized = normalizeKimiPayload(payload);
        const untrustedWindowIds = normalized.diagnostics.map((diagnostic) => diagnostic.code === "detail_invalid"
        ...
        const failure = error instanceof KimiFailure
            ? error
...
async function resolveCredential(broker, signal) {
    try {
...
async function resolveCliCredential(source, signal) {
    try {
...
function credentialFailureFor(resolution) {
...
function cliCredentialFailureFor(resolution) {
    if (resolution.status === "missing") {
...
function failureReport(failure, attempts, dependencies) {
    ...
    if (failure.staleEligible) {
        try {
            ...
            const stale = cached
                ? staleKimiReport(cached, failure.code, failure.retryAfter, attempts, dependencies.now())
                : undefined;
            if (stale)
                return stale;
        }
...
function staleKimiReport(cached, error, retryAfter, attempts, now) {
    if (cached.provider !== "kimi" ||
        cached.source !== "api" ||
    ...
    const windows = cached.windows.filter((window) => {
        ...
        const maxAgeSeconds = window.kind === "weekly" ? WEEK_SECONDS : FIVE_HOURS_SECONDS;
...
async function requestKimiQuota(apiKey, signal, fetchImplementation, now) {
    let response;
    try {
        response = await waitForDeadline(fetchImplementation(KIMI_QUOTA_URL, {
            method: "GET",
            headers: {
...
function rejectHttpFailure(response, receivedAt) {
    ...
    if (status === 429) {
        throw new KimiFailure("provider_rate_limited", {
            status: "rate_limited",
            staleEligible: true,
            retryAfter: normalizeRetryAfter(response.headers.get("retry-after"), receivedAt),
        });
    }
...
async function readBoundedBody(response, signal, lifetime) {
...
async function readBodyChunk(reader, signal, lifetime) {
...
function createResponseBodyLifetime(response) {
    let consumed = false;
    let cancellation;
    return {
        markConsumed() {
...
export function normalizeKimiPayload(payload) {
...
function normalizeDetail(value) {
    ...
    const used = explicitUsed !== undefined
        ? explicitUsed
        : remaining !== undefined
...
function normalizeWindowSeconds(value) {
    ...
    if (!window || typeof window.timeUnit !== "string")
        return undefined;
...
function normalizedReset(detail) {
...
function normalizeRfc3339(value) {
    ...
    const instant = local.getTime() - offsetMilliseconds + (second === 60 ? 1_000 : 0);
...
export function normalizeRetryAfter(value, receivedAt) {
...
function parseHttpDate(value, receivedAt) {
    const imf = /^(Sun|Mon|Tue|Wed|Thu|Fri|Sat), (\d{2}) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (\d{4}) (\d{2}):(\d{2}):(\d{2}) GMT$/.exec(value);
    if (imf) {
        return validatedHttpInstant(SHORT_WEEKDAYS.indexOf(imf[1]), Number(imf[4]), MONTHS.indexOf(imf[3]) + 1, Number(imf[2]), Number(imf[5]), Number(imf[6]), Number(imf[7]));
    }
    ...
    if (rfc850) {
        ...
        if (year > currentYear + 50)
            year -= 100;
        return validatedHttpInstant(LONG_WEEKDAYS.indexOf(rfc850[1]), year, MONTHS.indexOf(rfc850[3]) + 1, Number(rfc850[2]), Number(rfc850[5]), Number(rfc850[6]), Number(rfc850[7]));
    }
    const asctime = /^(Sun|Mon|Tue|Wed|Thu|Fri|Sat) (Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec) (?: (\d)|(\d{2})) (\d{2}):(\d{2}):(\d{2}) (\d{4})$/.exec(value);
    if (!asctime)
        return undefined;
    return validatedHttpInstant(SHORT_WEEKDAYS.indexOf(asctime[1]), Number(asctime[8]), MONTHS.indexOf(asctime[2]) + 1, Number(asctime[3] ?? asctime[4]), Number(asctime[5]), Number(asctime[6]), Number(asctime[7]));
}
function validatedHttpInstant(weekday, year, month, day, hour, minute, second) {
    if (weekday < 0 ||
        day < 1 ||
        day > daysInMonth(year, month) ||
        hour > 23 ||
...
function localTransportCode(error) {
...
function waitForDeadline(promise, signal) {
    if (signal.aborted) {
        return Promise.reject(new KimiFailure("request_timeout", { staleEligible: true }));
    }
    return new Promise((resolve, reject) => {
        const abort = () => reject(new KimiFailure("request_timeout", { staleEligible: true }));
        signal.addEventListener("abort", abort, { once: true });
        promise.then((value) => {
            signal.removeEventListener("abort", abort);
            resolve(value);
