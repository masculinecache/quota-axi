...
export function renderQuotaToon(response, binPath, full) {
    ...
    const windows = response.providers.flatMap((provider) => provider.windows.map((window) => ({
        provider: provider.provider,
        id: window.id,
        label: window.label,
    ...
    const effective = response.providers.flatMap((provider) => {
        ...
        return semantics.effectiveAvailability.map((availability) => ({
            ...
            effectivePercentRemaining: availability.effectivePercentRemaining ?? "unknown",
            boundedBy: availability.boundedBy.join(" + ") || "none",
            limitingWindowIds: availability.limitingWindowIds?.join(" + ") ?? "unknown",
            runway: availability.runway?.status ?? "unknown",
            usableRunwaySeconds: availability.runway?.usableRunwaySeconds ?? "unknown",
            projectedExhaustedAt: availability.runway?.projectedExhaustedAt ?? "unknown",
            limitingWindowId: availability.runway?.limitingWindowId ?? "unknown",
            projectionConfidence: availability.runway?.projectionConfidence ?? "unknown",
            projectionBasis: availability.runway?.projectionBasis ?? "unknown",
            unmeasurableWindowIds: availability.runway?.unmeasurableWindowIds?.join(" + ") ?? "none",
            unresolvedWindowIds: semantics.unresolvedWindowIds?.join(" + ") ?? "none",
    ...
    if (advice.length > 0)
        blocks.push(encode({ advice }));
    if (full) {
        const windowPace = response.providers.flatMap((provider) => provider.windows.map((window) => ({
            provider: provider.provider,
            id: window.id,
            ...
            projectionConfidence: window.pace?.projectionConfidence ?? "unknown",
            projectionBasis: window.pace?.projectionBasis ?? "unknown",
        })));
        const effectivePace = response.providers.flatMap((provider) => (provider.quotaSemantics?.effectiveAvailability ?? []).map((availability) => ({
            provider: provider.provider,
            scope: availability.scope,
            pace: availability.pace?.status ?? "unknown",
            aheadWindowIds: availability.pace?.aheadWindowIds?.join(" + ") ?? "none",
            unknownWindowIds: availability.pace?.unknownWindowIds?.join(" + ") ?? "none",
            worstReserve: availability.pace?.worstReservePercentPoints ??
                "unknown",
            worstReserveWindowId: availability.pace?.worstReserveWindowId ?? "unknown",
        })));
        ...
        const attempts = response.providers.flatMap((provider) => (provider.attempts ?? []).map((attempt) => attemptRow(provider, attempt)));
...
export function renderAuthToon(reports, binPath) {
    const sources = reports.flatMap((report) => report.sources.map((source) => ({
...
export function renderModelsToon(response, binPath, full) {
    const models = response.models.map((model) => ({
        ...
        effectivePercentRemaining: model.effective?.effectivePercentRemaining ?? "unknown",
        runway: model.effective?.runway?.status ?? "unknown",
        usableRunwaySeconds: model.effective?.runway?.usableRunwaySeconds ?? "unknown",
    }));
    const blocks = [
        encode({
            bin: collapseHome(binPath),
    ...
    if (response.sort)
        blocks.push(encode({ sort: response.sort }));
    ...
    if (full) {
        const evidence = response.models.map((model) => ({
            ...
            limitingWindowIds: model.effective?.limitingWindowIds?.join(" + ") ?? "unknown",
            projectedExhaustedAt: model.effective?.runway?.projectedExhaustedAt ?? "unknown",
...
export function redactedResponse(response, full) {
    if (full)
        return response;
    return {
        ...response,
...
function attemptRow(provider, attempt) {
    return {
        provider: provider.provider,
