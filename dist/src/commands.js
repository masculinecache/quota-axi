...
import { redactedResponse, renderAuthToon, renderModelsToon, renderQuotaToon, } from "./render.js";
import { formatInterval, runLiveTui } from "./tui-live.js";
import { detectTuiColorDepth, renderQuotaTui } from "./tui.js";
const DEFAULT_REFRESH_SECONDS = 300;
export async function quotaCommand(args, context) {
    ...
    if (flags.tui)
        return quotaTuiReport(flags, options);
 ...
 */
async function quotaTuiReport(flags, options) {
    const frame = (response, footerHint) => renderQuotaTui(redactedResponse(response, flags.full), {
 ...
 */
async function loadQuota(providers, options, live) {
...
export async function modelsCommand(args, context) {
    const binPath = context?.binPath ?? "quota-axi";
    ...
    const modelProviders = quota.providers.filter((provider) => MODEL_CATALOG_PROVIDER_IDS.includes(provider.provider));
    if (modelProviders.every(isFailed))
        process.exitCode = 1;
    return flags.json
        ? JSON.stringify(response, null, 2)
        : renderModelsToon(response, binPath, flags.full);
}
export async function authCommand(args, context) {
    const binPath = context?.binPath ?? "quota-axi";
    const flags = parseFlags(args);
    if (flags.tui) {
...
export async function fetchQuota(providers, options) {
