...
import { authCommand, modelsCommand, quotaCommand, } from "./commands.js";
import { VERSION } from "./version.js";
export const DESCRIPTION = "Report local agent-provider quota windows and model quota evidence.";
...
export async function main(options = {}) {
    ...
    await runAxiCli({
        ...
        getCommandHelp: (command) => command === "quota" || command === "auth" || command === "models"
            ? TOP_HELP
            : undefined,
    });
}
 ...
 */
export function normalizeArgv(raw) {
    if (raw.length === 0)
        return ["quota"];
