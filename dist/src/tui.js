 ...
 */
export function detectTuiColorDepth(env, isTty) {
    ...
    if ((env.TERM ?? "").includes("256color") || force === "2")
        return "256";
    return "16";
}
export function renderQuotaTui(response, options = {}) {
    const columns = Math.min(MAX_COLUMNS, Math.max(MIN_COLUMNS, options.columns ?? TWO_COLUMN_MIN));
...
function headerText(response, timeZone) {
    const live = response.providers.filter(isLive).length;
    const signedOut = response.providers.filter((provider) => provider.state.status === "auth_required").length;
    ...
    if (failed > 0)
        parts.push(`${failed} unavailable`);
    return parts.filter(Boolean).join(" · ");
}
function buildCard(provider, generatedAtMs) {
    return isLive(provider)
        ? buildLiveCard(provider, generatedAtMs)
        : buildFailedCard(provider);
}
function buildLiveCard(provider, generatedAtMs) {
    ...
    for (const note of cardNotes(provider)) {
        lines.push(interior([{ text: `   ${truncate(note, CARD_INTERIOR - 4)}`, style: "dimmer" }], "border"));
    }
    lines.push(interior([], "border"));
 ...
 */
function effectiveHeadline(provider, headline, stale) {
    const lines = [];
    ...
    const percentText = effectivePct === undefined ? undefined : `${Math.round(effectivePct)}%`;
    const headlineLabelWidth = Math.max(0, EFFECTIVE_BAR_WIDTH -
        lineWidth(verdict) -
        1 -
        displayWidth(percentText ?? "") -
        1);
    const left = effectivePct !== undefined && percentText !== undefined
        ? [
            {
                text: percentText,
 ...
 */
function hasWhollyUnknownWindowRelationships(provider) {
...
function windowsOnlyHeadline(stale) {
    const left = [
        {
    ...
    const right = [{ text: "no combined bound", style: "dim" }];
...
function buildFailedCard(provider) {
    const status = provider.state.status;
    const rightTitle = status === "auth_required" ? "signed out" : humanize(status);
    const lines = [
        titleLine({ text: ` ○ ${provider.provider} `, style: "dimBold" }, rightTitle, "borderDim"),
        interior([], "borderDim"),
    ];
    const message = humanize(provider.state.error ?? "") ||
...
function titleLine(name, rightText, borderStyle) {
    let right = rightText === "" ? "" : ` ${rightText} `;
    let dashes = CARD_WIDTH - 4 - displayWidth(name.text) - displayWidth(right);
    if (dashes < 1) {
        right = ` ${truncate(rightText, Math.max(0, CARD_WIDTH - 7 - displayWidth(name.text)))} `;
        dashes = Math.max(1, CARD_WIDTH - 4 - displayWidth(name.text) - displayWidth(right));
    }
    return [
...
function bottomLine(borderStyle) {
    return [{ text: `╰${"─".repeat(CARD_INTERIOR)}╯`, style: borderStyle }];
}
function interior(content, borderStyle) {
...
function windowRow(window, generatedAtMs) {
 ...
 */
export function thinBar(percentRemaining, markerPercent, width) {
    const fillStyle = percentRemaining === undefined ? "track" : healthStyle(percentRemaining);
    let halfUnits = 0;
    if (percentRemaining !== undefined) {
        const pct = Math.min(100, Math.max(0, percentRemaining));
        halfUnits = Math.round((pct / 100) * width * 2);
        if (pct > 0 && halfUnits === 0)
            halfUnits = 1;
        if (pct < 100 && halfUnits === width * 2)
            halfUnits = width * 2 - 1;
    }
    const cells = [];
    for (let index = 0; index < width; index++) {
        ...
        if (cellHalves === 2)
            cells.push({ text: "━", style: fillStyle });
        else if (cellHalves === 1)
            cells.push({ text: "╸", style: fillStyle });
        else
            cells.push({ text: "─", style: "track" });
    }
    if (markerPercent !== undefined && Number.isFinite(markerPercent)) {
        const cell = Math.min(width - 1, Math.max(0, Math.round((markerPercent / 100) * width)));
        cells[cell] = { text: "┃", style: "marker" };
    }
    return coalesce(cells);
}
function pickHeadlineAvailability(provider) {
    const availability = provider.quotaSemantics?.effectiveAvailability ?? [];
    return (availability.find((entry) => entry.scope.startsWith("all_") && entry.status === "known") ??
        availability.find((entry) => entry.status === "known") ??
        availability[0]);
}
function effectiveMarkerPercent(provider, headline) {
    if (!headline)
        return undefined;
 ...
 */
export function headlineLabel(provider, headline, width = HEADLINE_LABEL_WIDTH) {
    ...
    const suffix = scope !== undefined && !scope.startsWith("all_")
    ...
    if (displayWidth(joined) > width) {
        ...
        windowLabel = `${compactHeadlineWindowName(names[0], width - displayWidth(tie))}${tie}`;
    }
    return displayWidth(`${windowLabel}${suffix}`) <= width
        ? `${windowLabel}${suffix}`
        : windowLabel;
}
function compactHeadlineWindowName(label, width) {
    const safeLabel = sanitizeTerminalText(label);
    if (displayWidth(safeLabel) <= width)
        return safeLabel;
    const parts = safeLabel.match(/^(.*\S)\s+(\S+)$/u);
    if (parts === null)
        return truncate(safeLabel, width);
    const period = parts[2];
    const separatorWidth = 1;
    const prefixWidth = width - displayWidth(period) - separatorWidth;
    if (prefixWidth <= 0)
        return truncate(period, width);
 ...
 */
export function shortWindowLabel(window) {
    const tokens = window.label.split(/[\s_]+/).filter(Boolean);
    if (tokens.length > 1 &&
        /^(week|window|day|month|session|usage|quota)$/i.test(tokens[tokens.length - 1])) {
        tokens.pop();
    }
    ...
    if (displayWidth(label) > 7)
        label = truncate(label, 7);
    return label || truncate(window.id, 7);
}
function resetCountdown(window, generatedAtMs) {
...
function formatHeaderTime(iso, timeZone) {
...
function fullFooterLines(provider, width) {
    const accountParts = [provider.provider];
    const protectedAccountParts = new Set([0]);
    if (provider.account?.email)
        accountParts.push(provider.account.email);
    ...
    const attempts = (provider.attempts ?? []).map((attempt) => `${attempt.source} (${attempt.status}${attempt.error ? `: ${attempt.error}` : ""})`);
    const tried = attempts.length > 0 ? attempts : provider.state.sourcesTried;
    const completeParts = [...accountParts];
    if (tried.length > 0)
        completeParts.push(`tried ${tried.join(" → ")}`);
    const complete = completeParts.join(" · ");
    if (displayWidth(complete) <= width)
        return [complete];
    const lines = [fitFooterParts(accountParts, width, protectedAccountParts)];
    if (provider.attempts && provider.attempts.length > 0) {
        lines.push(...provider.attempts.map((attempt) => formatAttemptLine(attempt, width)));
    }
    else {
        lines.push(...provider.state.sourcesTried.map((source) => truncate(`  tried ${source}`, width)));
    }
    return lines;
}
function formatAttemptLine(attempt, width) {
    const prefix = "  tried ";
    ...
    if (displayWidth(complete) <= width)
        return complete;
    const fixedWidth = displayWidth(prefix) + displayWidth(statusPrefix) + displayWidth(suffix);
    ...
    const errorWidth = attempt.error
        ? Math.min(displayWidth(attempt.error), Math.max(1, available - minimumSourceWidth))
        : 0;
    const sourceWidth = Math.max(1, available - errorWidth);
    return `${prefix}${truncate(attempt.source, sourceWidth)}${statusPrefix}${attempt.error ? truncate(attempt.error, errorWidth) : ""}${suffix}`;
}
function fitFooterParts(parts, width, protectedParts = new Set([0])) {
    const separator = " · ";
    const complete = parts.join(separator);
    if (displayWidth(complete) <= width)
        return complete;
    const available = Math.max(0, width - displayWidth(separator) * (parts.length - 1));
    const widths = new Array(parts.length).fill(0);
    ...
    while (pending.length > 0) {
        ...
        const fitting = pending.filter((index) => displayWidth(parts[index]) <= share);
        if (fitting.length === 0) {
...
function layoutCards(cards, twoColumn) {
    const lines = [];
    if (!twoColumn) {
...
function truncate(text, width) {
    ...
    if (width <= 0)
        return "";
    if (displayWidth(safeText) <= width)
        return safeText;
    if (width === 1)
        return "…";
    const limit = width - 1;
    ...
    for (const unit of terminalTextUnits(safeText)) {
        ...
        if (unitWidth > 0 && used + unitWidth > limit)
            break;
        result += unit;
        used += unitWidth;
    }
...
function padBetween(left, right, width) {
    ...
    const pad = Math.max(1, width - lineWidth(boundedLeft) - lineWidth(boundedRight));
...
function terminalTextUnits(text) {
...
function terminalUnitWidth(unit) {
    if (/\p{Emoji_Presentation}/u.test(unit) ||
        (/\p{Emoji}/u.test(unit) &&
...
function styleSgr(style, depth) {
    const spec = style.startsWith("accent:")
        ? ACCENTS[style.slice("accent:".length)]
        : STYLES[style];
    const codes = [];
    if (spec.bold)
        codes.push("1");
